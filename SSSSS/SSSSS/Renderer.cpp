...
{
	//temp
	mVertShader = Shader(Shader::VertexShader, "standard.vert");
	mFragShader = Shader(Shader::FragmentShader, "standard.frag");
	vertices = {
...
void Renderer::initVulkan() {
	...
	createFramebuffers();
	createTextureImage();
	//this is the image view for textures
	createTextureImageView();
	createTextureSampler();
	createVertexBuffer();
	createIndexBuffer();
	createUniformBuffers();
	createDescriptorPool();//relate to swap chain image size
	createDescriptorSets();//relate to swap chain image size
...
void Renderer::cleanupSwapChain() {
	...
	vkDestroyDescriptorPool(device, descriptorPool, nullptr);//descriptor pool is related to swap chain images size
}
...
void Renderer::createGraphicsPipeline() {
	//modified by myself, use shaderc to compile vulkan shader in run time
	if (!mVertShader.CreateShader()) {
		throw std::runtime_error("failed to create pipeline layout! shader failed to compile: " + mVertShader.GetFileName());
	}

	if (!mFragShader.CreateShader()) {
		throw std::runtime_error("failed to create pipeline layout! shader failed to compile: " + mFragShader.GetFileName());
	}

	//load the binary shader file
	VkShaderModule vertShaderModule = createShaderModule(mVertShader.GetShaderBytecode());
	VkShaderModule fragShaderModule = createShaderModule(mFragShader.GetShaderBytecode());
...
void Renderer::createTextureImage() {
	...
	stbi_uc* pixels = stbi_load("rex.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
